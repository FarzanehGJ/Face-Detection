...
testing_data= np.stack((X_test, y_test, G_test, R_test), axis = -1)

# save testing data numpy array to the disk to be later used to rerun testing and generating evaluation report
np.save(Hyperparameters.ROOT_PATH + r'/Processed Dataset/Numpy/TestingImageSet.npy', testing_data)

# image transformations for training
training_transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.RandomResizedCrop(Hyperparameters.IMG_SIZE),
...
CrossValidationFunctions.k_fold_validation(Hyperparameters.N_SPLITS, X_train, y_train, training_transform, validation_transform=testing_transform)
