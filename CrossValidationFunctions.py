...
def k_fold_validation(n_splits, X_train, y_train, training_transform, validation_transform) :
    ...
    np.save(Hyperparameters.ROOT_PATH + r'/Processed Dataset/Numpy/CrossValidationEvaluations.npy', evaluations)
    ...
    plt.savefig('{}.png'.format(n_splits))
...
def train(iteration, training_data, testing_data, training_transform, testing_transform):
    ...
    testing_image_set = ToDataLoader.Imageset(testing_data, transform = testing_transform)
    testing_loader = DataLoader(testing_image_set, Hyperparameters.BATCH_SIZE)

    #if the function is not used for training purposes
    if iteration==-1:
        ...
        africa_testing_data = []
        europe_testing_data = []
        for i in range(0, int(testing_data.size / 4)):
            if testing_data[i, 2] == 5:
                male_testing_data.append(testing_data[i, 0:2])
            elif testing_data[i, 2] == 6:
                female_testing_data.append(testing_data[i, 0:2])
            if testing_data[i, 3] == 7:
                asia_testing_data.append(testing_data[i, 0:2])
            elif testing_data[i, 3] == 8:
                africa_testing_data.append(testing_data[i, 0:2])
            elif testing_data[i, 3] == 9:
                europe_testing_data.append(testing_data[i, 0:2])
        # set loaders for each category being tested
        ...
        #Put all loaders in a list to loop through ending with the complete list
        test_loader_list=[male_test_loader,female_test_loader,asia_test_loader,africa_test_loader,europe_test_loader,testing_loader ]
    else:
        test_loader_list = [testing_loader]
    ...
    for testing_loader in test_loader_list:
        ...
        # model testing
        with torch.no_grad():
            # Loop through all testing sets for all biases
        ...
        evaluation_parameters = evaluation(iteration, category, training_accuracy, training_loss, predicted_label, test_label, correct, total)
...
def evaluation(iteration, category, training_accuracy, training_loss, predicted_label, test_label, correct, total):
    ...
    if device == Hyperparameters.DEVICE_CUDA:
        test_label = test_label.to(Hyperparameters.DEVICE_CPU)
        predicted_label = predicted_label.to(Hyperparameters.DEVICE_CPU)
    ...
    plt.savefig('{}-{}.png'.format(iteration, category))
    
    # save final testing evaluation results for later redisplay
    if iteration == -1:
        np.save(Hyperparameters.ROOT_PATH + r'/Processed Dataset/Numpy/FinalTestingEvaluations.npy',
